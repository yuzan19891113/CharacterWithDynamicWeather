...
namespace MagicaCloth
{
    ...
    public class PhysicsManagerTeamData : PhysicsManagerAccess
    {
        ...
        public struct WorldInfluence
        {
            ...
            /// 速度制限を行うため必要
            /// </summary>
            public float clampRotationVelocityLimit;    // 0.0f = 無効
            public CurveParam clampRotation;
        }
        ...
        public void SetWorldInfluence(int teamId, float maxSpeed, float maxRotatinSpeed, BezierParam moveInfluence, BezierParam rotInfluence,
            ...
            BezierParam clampRotation, float clampRotatinVelocityLimit
            )
        {
            ...
            data.clampRotation = new CurveParam(clampRotation);
            data.clampRotationVelocityLimit = clampRotatinVelocityLimit;
        ...
        public void SetClampRotation(int teamId, BezierParam clampRotation, float velocityLimit)
        {
            var data = teamWorldInfluenceList[teamId];
            data.clampRotation = new CurveParam(clampRotation);
            data.clampRotationVelocityLimit = velocityLimit;
            teamWorldInfluenceList[teamId] = data;
        }
        ...
        struct PreProcessTeamDataJob : IJobParallelFor
        {
            ...
            public void Execute(int teamId)
            {
                ...
                else
                {
                    ...
                    // ClampRotationによる最大速度補正(v1.9.1)
                    // 移動速度の上昇と比例して速度制限をかける
                    if (wdata.clampRotationVelocityLimit > 1e-06f)
                    {
                        var maxClampAngle = wdata.clampRotation.Evaluate(0.5f); // 平均
                        var ratio = math.pow(MathUtility.Clamp01(maxClampAngle / 90.0f), wdata.clampRotationVelocityLimit);
                        //Debug.Log($"ratio:{ratio}");

                        // 移動
                        var clampMoveSpeed = math.max(ratio * 10.0f, 1.0f); // 最低(1m/s)
                        maxMoveSpeed = math.min(maxMoveSpeed, clampMoveSpeed);
                        //Debug.Log($"maxClampAngle:{maxClampAngle} clampMoveSpeed:{clampMoveSpeed} maxMoveSpeed:{maxMoveSpeed}");
