// Unity built-in shader source. Copyright (c) 2016 Unity Technologies. MIT license (see license.txt)
...
#ifndef TT_UNITY_CG_INCLUDED
    #define TT_UNITY_CG_INCLUDED
    ...
    inline half2 LinearToGammaSpace2(half2 linRGB)
    {
        #if BRDF0
            // Exact version, useful for debugging.
            return half2(LinearToGammaSpaceExact(linRGB.r), LinearToGammaSpaceExact(linRGB.g));
        #else
            linRGB = max(linRGB, half2(0.h, 0.h));
            // An almost-perfect approximation from http://chilliant.blogspot.com.au/2012/08/srgb-approximations-for-hlsl.html?m=1
            return max(1.055h * pow(linRGB, 0.416666667h) - 0.055h, 0.h);
        #endif
    }

    half4 tex2DN(sampler2D tex, float2 uv)
    {
        half4 color = tex2D(tex, uv);
            ...
            return color; 
        #else
            color.rg = LinearToGammaSpace2(color.rg);
            return color;
        #endif
    }
