...
namespace MagicaCloth
{
    ...
    public class ColliderCollisionConstraint : PhysicsManagerConstraint
    {
        ...
        struct CollisionJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    for (int j = 0; j < c.useLength; j++, dataIndex++)
                    {
                        ...
                        {
                            // カプセルコライダー判定
                            dist = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(1, 0, 0), out n);
                        }
                        else if (cflag.IsFlag(PhysicsManagerParticleData.Flag_CapsuleY))
                        {
                            // カプセルコライダー判定
                            dist = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(0, 1, 0), out n);
                        }
                        else if (cflag.IsFlag(PhysicsManagerParticleData.Flag_CapsuleZ))
                        {
                            // カプセルコライダー判定
                            dist = CapsuleColliderDetection(ref nextpos, basepos, radius, cindex, new float3(0, 0, 1), out n);
                        }
                        ...
                        {
                            // 球コライダー判定
                            dist = SphereColliderDetection(ref nextpos, basepos, radius, cindex, out n);
                        }
            ...
            /// <returns></returns>
            float SphereColliderDetection(ref float3 nextpos, float3 basepos, float radius, int cindex, out float3 normal)
            {
            ...
            /// <param name="friction"></param>
            /// <returns></returns>
            float CapsuleColliderDetection(ref float3 nextpos, float3 basepos, float radius, int cindex, float3 dir, out float3 normal)
            {
                var cpos = nextPosList[cindex];
