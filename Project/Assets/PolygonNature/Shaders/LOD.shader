...
Shader "SyntyStudios/LOD"
{
	Properties
	{
		...
		//_SnowStrength1("Snow Strength1", Range(0,1))=1 //Ñ©µÄÇ¿¶È
	}
	...
	SubShader
	{
		...
		void surf( Input i , inout SurfaceOutputStandard o )
		{
			...
			float3 albedo = ( tex2DNode2 * lerpResult162 ).rgb;

			float snowLerp = 0.9 * _SnowStrength;
			fixed3 sc = fixed3(1.0,0.98,0.98);
			albedo = lerp(albedo, sc, snowLerp);
			o.Albedo = albedo;
