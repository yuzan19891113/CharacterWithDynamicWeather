    ...
    public class PhysicsManagerParticleData : PhysicsManagerAccess
    {
        ...
        struct CopyBoneToParticleJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                {
                    // 移動影響
                    float moveRatio = (1.0f - wdata.moveIgnoreRatio) * (1.0f - moveInfluence) + wdata.moveIgnoreRatio;
                    float3 offpos = wdata.moveOffset * moveRatio;

                    // 回転影響
                    float rotRatio = (1.0f - wdata.rotationIgnoreRatio) * (1.0f - rotInfluence) + wdata.rotationIgnoreRatio;
